...
def main(highest_score):
    ...
    pygame.display.set_caption('SZTU Rush')

    sounds = {}
    for key, value in core.AUDIO_PATHS.items():
        sounds[key] = pygame.mixer.Sound(value)

    pygame.mixer.music.load(core.AUDIO_PATHS['bgm'])
    ...
    ch = Character([core.IMAGE_PATHS['ch'][0], core.IMAGE_PATHS['ch'][1]])  
    ...
    while True:
        ...
        if len(cloud_sprites_group) < 5 and random.randrange(0, 300) == 10:
            cloud_sprites_group.add(Cloud(core.IMAGE_PATHS['cloud'], position=(core.SCREENSIZE[0], random.randrange(30, 200))))
        ...
        if add_obstacle_timer > random.randrange(80, 130):
            ...
            if random_value >= 0 and random_value <= 7:
                ...
                ptera_sprites_group.add(Ptera(core.IMAGE_PATHS['ptera'], position=(core.SCREENSIZE[0], random.choice(position_ys))))
        ...
        for item in cactus_sprites_group:
            if pygame.sprite.collide_mask(ch, item):
                ch.die(sounds)
        for item in ptera_sprites_group:
            if pygame.sprite.collide_mask(ch, item):
                ch.die(sounds)
        ...
        score_board = Scoreboard(score, core.FONT_PATHS['joystix'], position=(core.SCREENSIZE[0] * 0.88, core.SCREENSIZE[1] * 0.05))
        highest_score_board = Scoreboard(highest_score, core.FONT_PATHS['joystix'], position=(core.SCREENSIZE[0] * 0.72, core.SCREENSIZE[1] * 0.05), is_highest=True)
